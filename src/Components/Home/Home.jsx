...
function Home() {
  //State  creation 
  const [name,setName]=useState('Vijith')// String
  const [age,setAge]=useState(22)//number

  //Array of data
  const[dept,setDept]=useState(['MERN','Python','Java','Testing'])
  ...
  return (
    ...
    <h3>Batch Details</h3>
    {
      <div>
        <p>ID:{batch.id}</p>
        <p>Name:{batch.name}</p>
        <p>Start:{batch.start}</p>
        <p>End:{batch.end}</p>
      </div>
    }

    <Childcard jsonData={data.quotes}/>
    <Product jsonData={product.products}/>
    </div>
  )
}
